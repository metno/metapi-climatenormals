...
import models.{ ClimateNormalsMonthElementsResponse, ClimateNormalsMonthElement }

/**
 * Creating a JSON representation of climate normals month elements data
 */
class ClimateNormalsMonthElementsJsonFormat extends BasicJsonFormat {

  implicit val climateNormalsMonthElementWrites = Json.writes[ClimateNormalsMonthElement]

  implicit val climateNormalsMonthElementsResponseWrites: Writes[ClimateNormalsMonthElementsResponse] = (
    ...
    (JsPath \ ApiConstants.DATA_NAME).write[Seq[ClimateNormalsMonthElement]]
  )(unlift(ClimateNormalsMonthElementsResponse.unapply))
   ...
   */
  def format[A](start: DateTime, data: List[ClimateNormalsMonthElement])(implicit request: Request[A]): String = {
    val size = data.size
    val duration = new Duration(DateTime.now.getMillis() - start.getMillis())
    val response = new ClimateNormalsMonthElementsResponse(
      new URL(ApiConstants.METAPI_CONTEXT),
      "ClimateNormalsMonthElementsResponse",
      "v0",
